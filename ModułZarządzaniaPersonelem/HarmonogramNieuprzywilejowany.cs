...
{
    public partial class HarmonogramNieuprzywilejowany : Form
    {
        Thread th;
        public HarmonogramNieuprzywilejowany()
        {
            InitializeComponent();
        }
        ...
        private void HarmonogramNieuprzywilejowany_Load(object sender, EventArgs e)
        {
            ...
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["cn"].ConnectionString))
            {
                try
                {
                    foreach (DataGridViewRow dgv in dataGridView1.SelectedRows)
                    {
                       ...
                       "AND task_ID IN (select task_ID from Task where description like @description) AND dateSchedule = @dateSchedule AND scheduleStatus_ID IN (select scheduleStatus_ID from ScheduleStatus where nameStatus like @nameSchedule) " +
                       "AND hourOfExecution between @hourOfExecution AND @hourOfExecution";
                        ...
                        cmd.Parameters.AddWithValue("@dateSchedule", dateTimePicker1.Value.ToShortDateString());
                        cmd.Parameters.AddWithValue("@nameSchedule", Convert.ToString(dataGridView1.CurrentRow.Cells[1].Value));
                        ...
                        ModyfikacjaHarmonogramuPracownik mod = new ModyfikacjaHarmonogramuPracownik(ds);
        ...
        private void HarmonogramNieuprzywilejowany_FormClosing(object sender, FormClosingEventArgs e)
        {
